...
// Tamil Nadu corridor congestion weights
const corridorWeights: Record<string, number> = {
  'che-egmore-tnagar': 1.0,
  'che-tnagar-guindy': 0.95,
  'che-guindy-chromepet': 0.9,
  'che-velachery-omr': 0.85,
  'che-porur-guindy': 0.9,
  'che-central-anna-nagar': 0.8,
  'che-omr-sholinganallur': 0.75,
  'cbe-jn-gandhi': 0.85,
  'cbe-gandhi-peela': 0.8,
  'mdu-meena-periyar': 0.8,
  'nh45-che-villu': 0.7,
  'nh44-hosur-krishi': 0.75,
};
...
export class TrafficEngine {
  ...
  setMode(mode: 'realistic' | 'heavy' | 'light' | 'moderate') { this.mode = mode; }
  ...
  tick(): Map<string, SegmentState> {
    ...
    for (const seg of allSegments) {
      ...
      for (const cId of connected) {
        ...
        if (cState && cState.congestionLevel > 0.7) spillover += (cState.congestionLevel - 0.7) * 0.15;
      }
      const target = Math.min(1, Math.max(0, baseWeight * timeMult + signalPulse + randomNoise + spillover));
  ...
  getState(segmentId: string): SegmentState | undefined { return this.states.get(segmentId); }
  getAllStates(): Map<string, SegmentState> { return this.states; }
  getHistory(segmentId: string): number[] { return this.history.get(segmentId) ?? []; }

  getSensorReading(sensorId: string, segmentId: string): SensorReading {
    const state = this.states.get(segmentId);
    const noise = () => (Math.random() - 0.5) * 0.1;
    if (!state) return { sensorId, segmentId, vehicleCount: 0, avgSpacing: 50, speedEstimate: 60, queueLength: 0, timestamp: Date.now(), status: 'online' };
    return {
      sensorId, segmentId,
  ...
  predictCongestion(segmentId: string): { prediction: number; risk: 'low' | 'medium' | 'high' } {
    ...
    return { prediction, risk: prediction > 0.8 ? 'high' : prediction > 0.5 ? 'medium' : 'low' };
  }
